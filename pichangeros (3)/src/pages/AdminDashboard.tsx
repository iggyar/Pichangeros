...
export default function AdminDashboard() {
  ...
  const filteredCourts = courts.filter(c => (c.name.toLowerCase().includes(courtSearch.toLowerCase()) || c.owner.toLowerCase().includes(courtSearch.toLowerCase())) && (courtFilter === "all" || c.status === courtFilter));
  const filteredReservations = reservations.filter(r => r.player.toLowerCase().includes(reservationSearch.toLowerCase()) || r.court.toLowerCase().includes(reservationSearch.toLowerCase()) || r.id.toLowerCase().includes(reservationSearch.toLowerCase()));
  const filteredPayments = payments.filter(p => p.player.toLowerCase().includes(paymentSearch.toLowerCase()) || p.reservationId.toLowerCase().includes(paymentSearch.toLowerCase()) || p.court.toLowerCase().includes(paymentSearch.toLowerCase()));
  ...
  const confirmRejectCourt = () => {
    if (!rejectingCourtId) return;
    if (!rejectReason.trim()) {
      toast.error("Debes ingresar una razón para el rechazo");
      return;
    }
    setCourts(courts.map(c => c.id === rejectingCourtId ? { ...c, status: "rejected", rejectReason } : c));
                ...
                <Card className="border-slate-200 shadow-sm rounded-2xl">
                  <CardContent className="p-6">
                    ...
                    <p className="text-xs text-green-600 mt-2 flex items-center">
                      <Activity className="w-3 h-3 mr-1" /> +12 esta semana
                    </p>
                  </CardContent>
                </Card>
                <Card className="border-slate-200 shadow-sm rounded-2xl">
                ...
                <Card className="border-slate-200 shadow-sm rounded-2xl">
                  <CardContent className="p-6">
                    ...
                    <p className="text-xs text-red-500 mt-2 flex items-center">
                      <XCircle className="w-3 h-3 mr-1" /> 5% tasa de cancelación
                    </p>
                  </CardContent>
                </Card>
                <Card className="border-slate-200 shadow-sm rounded-2xl">
                  <CardContent className="p-6">
                    ...
                    <p className="text-xs text-green-600 mt-2 flex items-center">
                      <Activity className="w-3 h-3 mr-1" /> +15% vs semana anterior
                    </p>
                  </CardContent>
                </Card>
              </div>
                          ...
                          <td className="px-6 py-4 text-right space-x-2">
                            <Dialog>
                              ...
                              <DialogContent className="sm:max-w-md">
                                ...
                                {selectedUser && (
                                  <div className="space-y-4">
                                    <div><span className="font-bold">Nombre:</span> {selectedUser.name}</div>
                                    <div><span className="font-bold">Email:</span> {selectedUser.email}</div>
                                    <div><span className="font-bold">Celular:</span> {selectedUser.phone}</div>
                                    <div><span className="font-bold">Rol:</span> {selectedUser.role === 'owner' ? 'Dueño' : 'Jugador'}</div>
                                    <div><span className="font-bold">Estado:</span> {selectedUser.status === 'active' ? 'Activo' : 'Suspendido'}</div>
                                    <div><span className="font-bold">Registro:</span> {selectedUser.registeredAt}</div>
                                  </div>
                                )}
                              </DialogContent>
                            </Dialog>
                ...
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">Tiempo límite y porcentaje de reembolso</p>
                      </div>
                      <Button variant="outline" size="sm">Editar</Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
                      ...
                      <Button variant="outline" size="sm" className="text-red-600 border-red-200 hover:bg-red-50">Activar</Button>
                    </div>
                  </div>
