                      ...
                      {manualPayments.map((p) => {
                        ...
                        return (
                          <tr key={p.id} className="odd:bg-white even:bg-slate-50/50 hover:bg-emerald-50/10 transition-colors">
                            <td className="px-6 py-5 text-slate-600">{formattedDate} {p.timeSlot || p.time}</td>
                            <td className="px-6 py-5 font-semibold text-slate-900">{p.userName || p.player || "Usuario"}</td>
                            <td className="px-6 py-5 text-slate-600">{p.fieldName || p.court}</td>
                            <td className="px-6 py-5 font-bold text-slate-900">S/ {(p.price || p.montoTotal || 0).toFixed(2)}</td>
                            ...
                            <td className="px-6 py-5">
                              <Badge variant="outline" className={`font-bold text-xs px-2.5 py-1
                                ${(p.estadoPago === 'pendiente_verificacion' || p.estadoPago === 'En Verificación' || p.status === 'En Verificación') ? 'border-amber-300 text-amber-800 bg-amber-50' : ''}
                                ${(p.estadoPago === 'Pago verificado - Esperando llegada' || p.estadoPago === 'Pago confirmado' || p.status === 'Pago verificado - Esperando llegada') ? 'border-blue-300 text-blue-800 bg-blue-50' : ''}
                                ${(p.status === 'Completado' || p.status === 'Completada' || p.status === 'Llegó a la cancha' || p.estadoPago === 'Completado') ? 'border-green-300 text-green-800 bg-green-50' : ''}
                                ${p.estadoPago === 'Rechazado' ? 'border-red-300 text-red-800 bg-red-50' : ''}
                              `}>
                                {(p.estadoPago === 'pendiente_verificacion' || p.estadoPago === 'En Verificación' || p.status === 'En Verificación') && 'Pendiente de verificación'}
                                {(p.estadoPago === 'Pago verificado - Esperando llegada' || p.estadoPago === 'Pago confirmado' || p.status === 'Pago verificado - Esperando llegada') && 'Pago verificado - Esperando llegada'}
                                {(p.status === 'Completado' || p.status === 'Completada' || p.status === 'Llegó a la cancha' || p.estadoPago === 'Completado') && 'Completado'}
                                {p.estadoPago === 'Rechazado' && 'Rechazado'}
                                {!p.estadoPago && !p.status && 'Pendiente'}
                              </Badge>
                            </td>
                ...
                <div className="space-y-6 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {detailedCourt.photos && detailedCourt.photos.length > 0 ? (
                        detailedCourt.photos.map((pUrl: string, idx: number) => (
                        ...
                        <div className="col-span-2 text-center py-6 text-sm text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                          Sin fotos de la cancha
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {detailedCourt.entrancePhoto ? (
                      <div className="relative aspect-video rounded-xl overflow-hidden bg-slate-100 border border-slate-200 max-w-sm mx-auto">
                        <img src={detailedCourt.entrancePhoto} alt="Entrada del local" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      </div>
                    ) : (
                      <div className="text-center py-4 text-sm text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                        No se ha adjuntado foto de entrada (No disponible)
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-slate-500 font-medium select-all mt-0.5">{detailedCourt.ownerEmail || detailedCourt.ownerPhone || "Sín correo registrado"}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      ...
                      <span className="font-bold text-slate-800">{detailedCourt.openTime || "08:00"} - {detailedCourt.closeTime || "22:00"}</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  ...
                  {/* Boutons d'action / Inline states inside the modal */}
