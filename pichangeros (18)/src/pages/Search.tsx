      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2 shrink-0 group">
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-green-600 flex items-center justify-center text-white shadow-md shadow-green-600/20 group-hover:scale-105 transition-transform">
              <Trophy className="w-5 h-5 md:w-6 md:h-6 text-white" />
            </div>
            <span className="text-xl md:text-2xl font-black tracking-tight text-slate-900 block">Pichangeros</span>
          </Link>
          
          <div className="flex-1 max-w-2xl relative group">
            <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 md:w-5 h-4 md:h-5 text-slate-400 group-focus-within:text-green-600 transition-colors z-20 pointer-events-none" />
            <style>{`
              .pac-container {
            ...
            {isLoaded ? (
              ...
              >
                <Input 
                  placeholder="Buscar por distrito, dirección o cancha..." 
                  className="pl-10 md:pl-12 bg-slate-100 border-none focus-visible:ring-2 focus-visible:ring-green-600/20 focus-visible:bg-white rounded-2xl h-10 md:h-12 w-full transition-all text-sm md:text-base shadow-inner w-full"
                  value={searchTerm}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link to="/dashboard">
              <Button variant="ghost" className="text-slate-600 hover:text-green-600 rounded-xl px-2 md:px-4 border border-transparent hover:border-slate-100">
            ...
            {!user && (
              <>
                <Link to="/login">
                  <Button variant="outline" className="text-green-600 border-green-200 hover:bg-green-50 rounded-xl px-3 md:px-4 h-9 md:h-10 text-xs md:text-sm font-semibold select-none transition-all">
                    Iniciar sesión
                  </Button>
                </Link>
                <Link to="/register">
                  <Button className="bg-green-600 hover:bg-green-700 text-white border-none rounded-xl px-3 md:px-4 h-9 md:h-10 text-xs md:text-sm font-semibold select-none transition-all shadow-md shadow-green-600/10">
                    Registrarse
                  </Button>
                </Link>
              </>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-6">
              {filteredFields.map(field => (
                ...
                >
                  <Card className={`overflow-hidden transition-all duration-300 shadow-sm hover:shadow-md border border-slate-100 bg-white rounded-2xl relative ${highlightedCourt === field.id ? 'ring-2 ring-emerald-500' : ''}`}>
                    <div className="flex flex-col sm:flex-row h-full relative">
                      <div className="w-full sm:w-[38%] h-48 sm:h-auto relative overflow-hidden shrink-0">
                        <img 
                        ...
                        <Badge className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm text-slate-900 border-none px-2.5 py-1 flex items-center gap-1 shadow-sm rounded-lg">
                          <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                          <span className="font-bold text-xs">{field.rating ? field.rating.toFixed(1) : "Nuevo"}</span>
                        </Badge>
                      </div>
                      <div className="w-full sm:w-[62%] p-4 md:p-5 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start gap-2 mb-1.5">
                             <h3 className="font-bold text-base md:text-lg text-slate-900 group-hover:text-emerald-600 transition-colors line-clamp-1 tracking-tight">
                               {field.name}
                             </h3>
                          </div>
                          <p className="text-slate-500 text-xs md:text-sm flex items-center gap-1.5 mb-3 line-clamp-1">
                            <MapPin className="w-3.5 h-3.5 shrink-0 text-slate-400" />
                            {field.location?.address || field.address || "Dirección no especificada"}
                          </p>
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            <Badge variant="secondary" className="bg-slate-100 text-slate-600 hover:bg-slate-100 border-none rounded-lg px-2.5 py-0.5 text-[11px] font-semibold">
                              {field.type || "Fútbol"}
                            </Badge>
                            {(field.openTime || field.closeTime) && (
                              <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 hover:bg-emerald-50 border-none rounded-lg px-2.5 py-0.5 text-[11px] font-semibold flex items-center gap-1">
                                <Clock className="w-3 h-3" />
                                {formatTime12h(field.openTime || "08:00")} - {formatTime12h(field.closeTime || "22:00")}
                              </Badge>
                            )}
                          </div>
                        </div>
                        <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between">
                          <div className="flex flex-col">
                             <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider leading-none mb-1">Desde</span>
                             <div className="text-lg md:text-xl font-extrabold text-slate-900 leading-none">
                               S/ {field.pricePerHour || field.price} <span className="text-xs font-normal text-slate-400">/hr</span>
                             </div>
                          </div>
                          <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl h-9 px-4 text-xs font-bold shadow-sm border-none transition-all">
                            Reservar
                          </Button>
                        </div>
                      </div>
        ...
        <div className="hidden md:block w-full md:w-1/2 lg:w-3/5 bg-slate-200 relative z-0 h-[calc(100vh-80px)]">
          ...
          >
            ...
            />
            {filteredFields.map(field => {
              const latRaw = field.latitude || field.location?.lat;
              const lngRaw = field.longitude || field.location?.lng;
