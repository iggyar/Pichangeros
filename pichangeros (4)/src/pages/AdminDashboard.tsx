...
import { collection, query, getDocs, updateDoc, doc, onSnapshot, orderBy, where, setDoc, serverTimestamp } from "firebase/firestore";
...
export default function AdminDashboard() {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      for (const docSnapshot of snapshot.docs) {
        ...
        if (data.type === "review" && data.targetId) {
          const reviewDoc = await getDocs(query(collection(db, "reviews"), where("__name__", "==", data.targetId)));
          if (!reviewDoc.empty) {
            targetContent = reviewDoc.docs[0].data().comment || "Sin comentario (solo calificación)";
          }
        }
  ...
  useEffect(() => {
    const q = query(collection(db, "bookings"), where("method", "in", ["Yape", "Plin"]));
  ...
  useEffect(() => {
    const q = query(collection(db, "bookings"), where("paymentStatus", "in", ["paid", "released"]));
    const unsubscribe = onSnapshot(q, (snapshot) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      for (const docSnapshot of snapshot.docs) {
        ...
        if (data.ownerId) {
          const userDoc = await getDocs(query(collection(db, "users"), where("__name__", "==", data.ownerId)));
          if (!userDoc.empty) {
            ownerName = userDoc.docs[0].data().displayName || "Desconocido";
          }
        }
  ...
  const filteredCourts = courts.filter(c => (c.name || "").toLowerCase().includes(courtSearch.toLowerCase()) || (c.owner || "").toLowerCase().includes(courtSearch.toLowerCase()));
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card className="border-slate-200 shadow-sm rounded-2xl">
                  <CardContent className="p-6">
                    ...
                    <h3 className="text-2xl font-bold text-slate-900">S/ {(payments.reduce((acc, p) => acc + (p.price || 0), 0) * (systemSettings.commission / 100)).toFixed(2)}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="border-slate-200 shadow-sm rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-100">
                  <CardContent className="p-6">
                    ...
                    <h3 className="text-3xl font-bold text-blue-900">S/ {payments.reduce((acc, p) => acc + (p.price || 0), 0).toFixed(2)}</h3>
                  </CardContent>
                </Card>
                <Card className="border-slate-200 shadow-sm rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border-green-100">
                  <CardContent className="p-6">
                    ...
                    <h3 className="text-3xl font-bold text-green-900">S/ {(payments.reduce((acc, p) => acc + (p.price || 0), 0) * (systemSettings.commission / 100)).toFixed(2)}</h3>
                  </CardContent>
                </Card>
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {manualPayments.map((p) => {
                        const ownerPayout = (p.price || 0) * 0.9;
                        return (
                          <tr key={p.id} className="hover:bg-slate-50">
                            <td className="px-6 py-4 text-slate-600">{p.date} {p.time}</td>
                            ...
                            <td className="px-6 py-4 font-bold text-slate-900">S/ {(p.price || 0).toFixed(2)}</td>
                            <td className="px-6 py-4 font-mono text-xs font-bold text-blue-600">{p.yapeOperationCode || "N/A"}</td>
                            <td className="px-6 py-4">
                              <Badge variant="outline" className={`
                                ${p.estadoPago === 'Pago confirmado' ? 'border-green-200 text-green-700 bg-green-50' : ''}
                                ${p.estadoPago === 'Pago enviado - verificar' ? 'border-yellow-200 text-yellow-700 bg-yellow-50' : ''}
                            ...
                            <td className="px-6 py-4">
                              <div className="flex flex-col">
                                ...
                                <Badge variant="outline" className={`mt-1 h-5 text-[10px] w-fit ${p.pagoAlOwner ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                                  {p.pagoAlOwner ? "Pagado" : "Pendiente"}
                                </Badge>
                              </div>
                            </td>
                            <td className="px-6 py-4 text-right space-x-2">
                              {p.estadoPago === "Pago enviado - verificar" && (
                                <>
                              ...
                              {p.estadoPago === "Pago confirmado" && !p.pagoAlOwner && (
