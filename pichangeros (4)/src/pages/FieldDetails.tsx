...
import { MapPin, Star, Calendar as CalendarIcon, Clock, CheckCircle2, ChevronLeft, CreditCard, Lock, Flag, Trophy } from "lucide-react";
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "chats", chatId), {
        ...
        lastMessage: `Nueva reserva en ${field.name} para el ${format(selectedDate, "dd/MM/yyyy")} a las ${selectedTime}`,
      }, { merge: true });

      // Add initial messages
      await addDoc(collection(db, "chats", chatId, "messages"), {
        senderId: "system",
        text: "Normas de comunicación: Tratar a todos con respeto es obligatorio. Las ofensas o faltas de respeto serán sancionadas con la suspensión de la cuenta.",
        createdAt: serverTimestamp()
      });

      await addDoc(collection(db, "chats", chatId, "messages"), {
        senderId: "system",
        text: `Nueva reserva en ${field.name} para el ${format(selectedDate, "dd/MM/yyyy")} a las ${selectedTime}. El código de acceso es: ${securityCode}`,
        createdAt: serverTimestamp()
      });
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 text-white">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="flex flex-wrap items-center gap-4 text-slate-300">
               <p className="flex items-center gap-2 bg-white/5 backdrop-blur-sm px-4 py-2 rounded-2xl border border-white/10">
          ...
          <div className="lg:col-span-2 space-y-8 md:space-y-12">
            ...
            <section className="space-y-6">
               ...
               <div className="h-72 md:h-96 rounded-[2.5rem] overflow-hidden border border-slate-200 shadow-inner z-10">
                <MapContainer 
                  center={[field.location?.lat || -12.046374, field.location?.lng || -77.042793]} 
                  zoom={15} 
                  style={{ height: '100%', width: '100%' }}
                  scrollWheelZoom={false}
                >
                  <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                  <Marker position={[field.location?.lat || -12.046374, field.location?.lng || -77.042793]} icon={greenIcon}>
            ...
            <div className="lg:sticky lg:top-28 space-y-6 pb-24 md:pb-0">
              <Card className="rounded-[2.5rem] border-none shadow-2xl shadow-green-900/10 bg-white overflow-hidden p-0">
                <div className="bg-slate-950 p-8 text-white">
                   ...
                   <p className="text-5xl font-black">S/ {field.pricePerHour} <span className="text-sm font-bold text-slate-400 opacity-60 ml-1 italic">Incluye IGV</span></p>
                </div>
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <Dialog open={isBookingModalOpen} onOpenChange={setIsBookingModalOpen}>
                      ...
                      <DialogContent className="sm:max-w-md rounded-[2.5rem] border-none shadow-2xl p-8">
                        <DialogHeader>
                          <DialogTitle className="text-3xl font-black tracking-tight text-slate-900 mb-2 leading-none">Confirmar Reserva</DialogTitle>
                          <DialogDescription className="text-slate-500 font-medium pb-4 border-b border-slate-100">
                            Estás a un paso de asegurar tu pichanga.
                          </DialogDescription>
                        </DialogHeader>
                        
                        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 my-6 space-y-3">
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Cancha</span>
                            <span className="font-bold text-slate-900">{field.name}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Fecha</span>
                            <span className="font-bold text-slate-900">{format(selectedDate, "dd 'de' MMMM", { locale: es })}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Horario</span>
                          ...
                          <div className="pt-4 mt-4 border-t border-slate-200 flex justify-between items-center">
                            <span className="text-slate-900 font-black tracking-tight">Total a pagar:</span>
                            <span className="text-3xl font-black text-green-600">S/ {field.pricePerHour}</span>
                          </div>
                        </div>

                        <div className="space-y-4">
                          <h4 className="text-[10px] text-slate-400 font-black uppercase tracking-widest px-1">Método de pago</h4>
                          <div className="grid grid-cols-3 gap-2">
                            {[
                                { id: 'card', label: 'Tarjeta', icon: CreditCard, color: 'green' },
                                { id: 'yape', label: 'Yape', icon: CheckCircle2, color: 'purple' },
                                { id: 'plin', label: 'Plin', icon: CheckCircle2, color: 'blue' }
                            ].map((method) => (
                                <button 
                                  key={method.id}
                                  ...
                                  className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all ${
                                      ...
                                      ? `border-${method.color}-500 bg-${method.color}-50 text-${method.color}-700 shadow-sm` 
                        ...
                        <DialogFooter className="mt-8 flex flex-col gap-3 sm:flex-col">
                          <Button 
                            className="w-full h-14 bg-slate-950 hover:bg-slate-800 text-white rounded-2xl text-lg font-black shadow-xl transition-all active:scale-95" 
                            onClick={handleBooking}
                            disabled={bookingLoading}
                          >
                            {bookingLoading ? "Procesando..." : "Pagar y Confirmar"}
                          </Button>
                          <div className="flex items-center justify-center gap-2 text-[10px] text-slate-400 font-black uppercase tracking-widest w-full mt-4">
                            <Lock className="w-3.5 h-3.5" />
                            <span>Transacción Segura en la cancha</span>
                          </div>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                    ...
                    <p className="text-[10px] text-center text-slate-400 font-bold uppercase tracking-widest px-4 leading-relaxed">
                       Reserva 100% segura. Recibirás tu código de acceso al instante por correo y chat.
                    </p>
                  </div>
                </CardContent>
              </Card>
