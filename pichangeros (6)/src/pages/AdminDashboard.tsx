...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    setLoading(false);
  }, [user, isAdminEmail, authLoading, navigate]);
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      for (const docSnapshot of snapshot.docs) {
        ...
        if (!data.userName && data.userId) {
          try {
            const userSub = await getDocs(query(collection(db, "users"), where("__name__", "==", data.userId)));
            if (!userSub.empty) playerName = userSub.docs[0].data().displayName || playerName;
          } catch (e) {}
        }
        if (!data.fieldName && data.fieldId) {
          try {
            const fieldSub = await getDocs(query(collection(db, "fields"), where("__name__", "==", data.fieldId)));
            if (!fieldSub.empty) courtName = fieldSub.docs[0].data().name || courtName;
          } catch (e) {}
        }
