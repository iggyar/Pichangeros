...
import { MapPin, Star, Calendar as CalendarIcon, Clock, CheckCircle2, ChevronLeft, ChevronRight, CreditCard, Lock, Flag, Trophy, AlertCircle, Info, Upload, X, Image as ImageIcon, Video as VideoIcon } from "lucide-react";
...
export default function FieldDetails() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, async (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.ownerId) {
          getDoc(doc(db, "users", data.ownerId)).then((ownerSnap) => {
            if (ownerSnap.exists()) {
  ...
  const mediaItems: Array<{ type: 'image' | 'video'; url: string; label: string }> = [];
  if (field) {
    const photos: string[] = Array.isArray(field.photos) && field.photos.length > 0
      ? field.photos
      : [field.entrancePhoto || "https://images.unsplash.com/photo-1529900748604-07564a03e7a6?auto=format&fit=crop&q=80&w=1200"];
    
    photos.forEach((p, idx) => {
      if (p) mediaItems.push({ type: 'image', url: p, label: `Foto ${idx + 1}` });
    });

    if (field.entrancePhoto && !photos.includes(field.entrancePhoto)) {
      mediaItems.push({ type: 'image', url: field.entrancePhoto, label: "Entrada del local" });
    }

    if (field.videoUrl) {
      mediaItems.push({ type: 'video', url: field.videoUrl, label: "Video promocional" });
    }
  }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <section className="relative h-72 sm:h-96 w-full overflow-hidden">
        <div className="absolute inset-0 bg-slate-900 group">
          <img 
            src={mediaItems[activeMediaIndex]?.url || field.photos?.[0] || "https://images.unsplash.com/photo-1529900748604-07564a03e7a6?auto=format&fit=crop&q=80&w=1200"} 
            alt={field.name} 
              ...
              <section className="space-y-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-black text-slate-900 flex items-center gap-2">
                    <ImageIcon className="w-5 h-5 text-green-600" /> Galería de Fotos y Video
                  </h3>
                ...
                {/* Main Media Display */}
                <div className="relative aspect-video w-full rounded-2xl overflow-hidden bg-slate-950 border border-slate-200 shadow-lg group">
                  {mediaItems[activeMediaIndex]?.type === 'video' ? (
                    ...
                    <img 
                      src={mediaItems[activeMediaIndex]?.url || "https://images.unsplash.com/photo-1529900748604-07564a03e7a6?auto=format&fit=crop&q=80&w=1200"} 
                      alt={field.name} 
                  ...
                  {mediaItems.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 hover:bg-black/80 text-white flex items-center justify-center backdrop-blur-md transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 hover:bg-black/80 text-white flex items-center justify-center backdrop-blur-md transition-colors"
                      >
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-1 pt-1">
                    {mediaItems.map((item, idx) => (
                      <button
                        ...
                        className={`relative w-28 h-20 shrink-0 rounded-2xl overflow-hidden border-2 transition-all ${
                      ...
                      >
                        {item.type === 'video' ? (
                          <div className="w-full h-full bg-slate-900 flex flex-col items-center justify-center text-white p-1">
                            <VideoIcon className="w-5 h-5 text-green-400 mb-1" />
                            <span className="text-[10px] font-bold uppercase tracking-wider text-green-300">Video</span>
                          </div>
                        ) : (
                ...
                <CardContent className="p-8 space-y-8">
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <Dialog open={isBookingModalOpen} onOpenChange={setIsBookingModalOpen}>
                      ...
                      <DialogContent className="sm:max-w-md rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden z-[1000] max-h-[90vh] flex flex-col">
                        <div className="bg-slate-950 p-6 flex items-center justify-between text-white shrink-0">
                        ...
                        <div className="p-6 sm:p-8 space-y-6 overflow-y-auto max-h-[calc(90vh-70px)]">
                          <div className="text-center space-y-4">
                            ...
                            <div className="bg-purple-50 border border-purple-200/80 rounded-2xl p-3 text-center shadow-xs">
                              <p className="text-xs font-extrabold text-purple-950">
                                Pagando a: {ownerProfile?.displayName || ownerProfile?.businessName || field.owner || "Dueño de Cancha"} - {ownerProfile?.phoneNumber || ownerProfile?.paymentAccount || field.ownerPhone || "No registrado"}
                              </p>
                            </div>
                          </div>
